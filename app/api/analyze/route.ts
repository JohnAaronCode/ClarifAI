...
async function checkRateLimit(ip: string): Promise<{ allowed: boolean; remaining: number; resetIn: number }> {
  try {
    ...
    const key     = `clarifai:ratelimit:${ip}`
    const current = await redis.incr(key)
    if (current === 1) await redis.expire(key, RATE_LIMIT_WINDOW)
    const ttl       = await redis.ttl(key)
    const remaining = Math.max(0, RATE_LIMIT_MAX - current)
    return { allowed: current <= RATE_LIMIT_MAX, remaining, resetIn: ttl > 0 ? ttl : RATE_LIMIT_WINDOW }
  } catch (err) {
    ...
    return NextResponse.json(
      {
        verdict: "ERROR",
        confidence_score: 0,
        explanation: `Too many requests. You have reached the limit of ${RATE_LIMIT_MAX} analyses per minute. Please wait ${resetIn} seconds before trying again.`,
      },
      {
        status: 429,
        headers: {
...
function detectRepetitiveContent(content: string): { isRepetitive: boolean; reason: string } {
  ...
  if (words.length > 0 && new Set(words).size / words.length < 0.25) {
  ...
  for (const [word, freq] of wordFreq.entries()) {
    if (freq > maxFreq && word.length > 3) { maxFreq = freq; maxWord = word }
  }
  if (maxFreq > 0 && maxFreq / words.length > 0.4) {
  ...
  if (repeatedPhrases.size > words.length / 10) {
...
function buildSourceSearchUrl(sourceDomain: string, query: string): string {
  return buildSourceSearchUrlShared(sourceDomain, query)
}
...
async function findRelatedTrustedSources(
  ...
  const onlyArticles = (
    results: Array<{ name: string; url: string; article_url: string; search_url: string; homepage_url: string }>
  ): SourceResult[] =>
    results
      .filter(r => r.article_url && r.article_url !== r.homepage_url && r.article_url.startsWith("http"))
      .slice(0, 2)

  if (gNewsApiKey) {
    const results  = await fetchFromGNews(query, excludeDomain, gNewsApiKey)
    const articles = onlyArticles(results)
    if (articles.length > 0) { console.log(`[ClarifAI] GNews returned ${articles.length} real article links`); return articles }
  }
  ...
  if (googleSearchApiKey && googleSearchCx) {
    ...
    const articles = onlyArticles(results)
    if (articles.length > 0) { console.log(`[ClarifAI] Google Search returned ${articles.length} real article links`); return articles }
  }

  if (newsApiKey) {
    const results  = await fetchFromNewsAPI(query, excludeDomain, newsApiKey)
    const articles = onlyArticles(results)
    if (articles.length > 0) { console.log(`[ClarifAI] NewsAPI returned ${articles.length} real article links`); return articles }
  }

  console.log("[ClarifAI] No real related article links found — returning empty")
  return []
}
...
async function analyzeContentWithDualEngine(
  ...
  const evidence = buildEvidenceContext({ sourceAnalysis, factChecks, clickbaitScore, credibilityPatterns, sentimentAnalysis, detectedTopics })
  ...
  if (groqApiKey || openaiApiKey || process.env.GEMINI_API_KEY) {
  ...
  for (const src of relatedSources) {
    ...
    const isRealArticle = src.article_url && src.article_url !== src.homepage_url && src.article_url.startsWith("http")
  ...
  const sourceCredibility = await analyzeSourceCredibility(content, type, sourceAnalysis.credibility, verdict, originalInput)
  ...
  let articleSummary: ReturnType<typeof generateArticleSummary>
  if (ensembleResult?.ai_summary) {
    const wordCount = content.trim().split(/\s+/).length
    articleSummary = {
      summary:              ensembleResult.ai_summary.summary,
      key_claims:           ensembleResult.ai_summary.key_claims,
      word_count:           wordCount,
      reading_time_minutes: Math.max(1, Math.ceil(wordCount / 200)),
    }
  } else {
    articleSummary = generateArticleSummary(content)
  }
...
function buildEvidenceContext(data: {
  ...
  const validFactChecks = data.factChecks.filter((f: any) =>
    f.source !== "Source not identified" && f.reviewer !== "Manual Verification Required" && f.relevance > 0.1
  )
...
function extractEntitiesAdvanced(text: string) {
  ...
  const locationIndicators = new Set([
    "city","province","region","island","country","street","avenue","road",
    "bay","sea","ocean","river","lake","mountain","manila","cebu","davao",
    "luzon","visayas","mindanao","philippines","metro",
  ])
  for (let i = 0; i < words.length - 1; i++) {
    const w1 = words[i], w2 = words[i + 1]
    const pair = `${w1} ${w2}`
    if (!/^[A-Z][a-z]+\s+[A-Z][a-z]+/.test(pair)) continue
    if (w2.includes("Inc") || w2.includes("Corp") || w2.includes("Ltd") || w1 === "Department" || w1 === "Ministry" || w1 === "Bureau" || w1 === "Commission" || w1 === "Agency" || w1 === "Office") {
      organizations.add(pair)
    } else if (locationIndicators.has(w1.toLowerCase()) || locationIndicators.has(w2.toLowerCase())) {
      locations.add(pair)
    } else {
      persons.add(pair)
    }
...
async function generateFactChecksWithRealAPIs(content: string, claims: string[]) {
        ...
        if (res.ok) {
          ...
          if (data.articles?.length) {
            const relevant = data.articles
              .map((a: any) => ({
                claim:        a.title,
                conclusion:   a.description ? `${a.source?.name || "News source"} reports: "${a.description.substring(0, 120)}"` : `Covered by ${a.source?.name || "trusted news source"}`,
                source:       a.url,
  ...
  const sorted = results.filter(r => r.relevance > 0 || r.reviewer !== "Manual Verification Required").sort((a, b) => b.relevance - a.relevance)
...
async function analyzeSourceCredibility(content: string, type: string, baseCredibility: number, verdict?: string, originalInput?: string) {
  const googleApiKey = process.env.GOOGLE_FACT_CHECK_API_KEY
  const newsApiKey   = process.env.NEWS_API_KEY
  ...
  if (type === "url") {
    const urlToParse = originalInput || content
    try {
...
async function analyzeContentQuality(content: string, verdict?: string, confidence?: number) {
  ...
  const result: any = { readability_score: null, grammar_issues: [], coherence_score: null, structure: { paragraphs: content.split(/\n\s*\n/).length, avg_sentence_length: 0 }, quality_label: "Neutral" }
